...
class MupixObject():
	...
	@classmethod
	def partwise_from_filepath(cls, filepath):
		"""
		"""
		notes, rests, timeSignatures, keySignatures, clefs = [], [], [], [], []
		...
		# Necessary evil until refactor :(
		for part in range(parts_index):
			timeSignatures[part] = normalize_object_list(object_list=timeSignatures[part], total_measures=measuresInScore, total_parts=parts_index)
			keySignatures[part] = normalize_object_list(object_list=keySignatures[part], total_measures=measuresInScore, total_parts=parts_index)
			clefs[part] = normalize_object_list(object_list=clefs[part], total_measures=measuresInScore, total_parts=parts_index)

		# Necessary evil until refactor :(
		for part in range(parts_index):
			notes[part] = add_step_information(notes[part], keySignatures[part])  # only once keySignatures are normalized can we add the step information.
